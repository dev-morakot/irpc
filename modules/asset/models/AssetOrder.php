...
/**
 * This is the model class for table "asset_order".
 *
 * @property integer $id
 ...
 */
class AssetOrder extends \yii\db\ActiveRecord
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'full_name' => Yii::t('app','ชื่อผู้ขอเบิก'),
     ...
     */
    public function getAssetOrderLines() {
        return $this->hasMany(AssetOrderLine::className(), ['order_id' => 'id']);
    }
    ...
    public function approve(){
        $model = $this;
        $tx = AssetOrder::getDb()->beginTransaction();
        try {
            $prev_state = $model->state;
            $model->approver_id = Yii::$app->user->id;
            $model->state = AssetOrder::APPROVED;
            $model->date_approve = DateHelper::nowStringDB();
            if($model->save(false)) {
                $asLineList = AssetOrderLine::find()->where(['order_id' => $model->id])->all();
                $amount = 0;
                ...
                Yii::$app->userlog->info(
                    'เบิกพัสดุ-ครุภัณฑ์ id=' . $model->id . ', name=' . $model->name. ', state [' . $prev_state . '->' . $model->state . ']'
                ,AssetOrder::tableName(), 'update');
    ...
    {
        return new AssetOrderQuery(get_called_class());
    }
}
